...
@dataclass
class DownloadState:
    """Represents the persisted state of a single download."""
...
class ProgressDB:
    """Gestor de estado persistente usando SQLite con WAL mode."""
    ...
    def __del__(self):
        """Fallback para cerrar la conexión si close() no fue llamado"""
        try:
            self._conn.close()
        except Exception:
            pass
