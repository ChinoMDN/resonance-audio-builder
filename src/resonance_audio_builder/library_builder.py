...
class AudioDownloader:
    ...
    def download(self, search_result: SearchResult, track: TrackMetadata, stop_check: callable) -> DownloadResult:
        ...
        while not self.keyboard.should_quit():
            ...
            while attempt < self.cfg.MAX_RETRIES and not success:
                ...
                try:
                    ...
                    try:
                        result = self.downloader.download(search_result, track, lambda: self.keyboard.should_quit())
                    finally:
...
[bold]Quality:[/bold]   {self.cfg.MODE}"""
    ...
    def _select_csv(self) -> Optional[str]:
        ...
        if not csvs:
            console.print(
                Panel(
                    "[yellow]No CSV files found in the current directory.[/yellow]\n\nExport playlists from: [link=https://exportify.net/]https://exportify.net/[/link]",
                    title="Warning",
                    border_style="yellow",
                )
            )
