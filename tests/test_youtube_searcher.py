...
from resonance_audio_builder.audio.youtube import YouTubeSearcher, SearchResult
from resonance_audio_builder.core.exceptions import NotFoundError, RecoverableError
...
class TestYouTubeSearcher:
    ...
    def searcher(self, mock_youtube_api):
        ...
        with patch("resonance_audio_builder.audio.youtube.yt_dlp.YoutubeDL", return_value=mock_youtube_api), \
            ...
            # Allow run_in_executor to just call the function immediately for testing locallly (simulated sync)
            async def _fake_run(executor, func, *args):
                return func()
            mock_loop.return_value.run_in_executor = _fake_run
    ...
    async def test_search_by_text_success(self, searcher, mock_youtube_api):
        ...
        mock_youtube_api.extract_info.return_value = {
            "entries": [{
                "id": "vid1", "title": "Song", "uploader": "Artist", "duration": 200, "webpage_url": "url"
            }]
        }
