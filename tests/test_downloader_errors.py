...
class TestDownloaderFailures:
    ...
    async def test_download_yt_dlp_critical_failure(self, downloader):
        ...
        search_res = SearchResult(
            "vid", "Title", 100
        )  # Removed 'url' based on previous finding, wait SearchResult signature
    ...
    async def test_download_cleanup_on_failure(self, downloader, tmp_path):
        ...
        with (
            patch.object(downloader, "_download_raw", return_value=temp_file),
            patch.object(downloader, "_transcode", return_value=False),
        ):  # Transcode falla
