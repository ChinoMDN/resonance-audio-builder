...
class TestAudioDownloader:
    ...
    async def test_download_success_hq_only(self, downloader):
        ...
        # Mock raw download
        with (
            patch.object(downloader, "_download_raw", new_callable=AsyncMock) as mock_raw,
            ...
            with patch("pathlib.Path.exists", return_value=True), patch("pathlib.Path.stat") as mock_stat:
                ...
                res = await downloader.download(
                    SearchResult("vid", "Title", 100, "url"), TrackMetadata("id1", "Title", "Artist")
                )
    ...
    async def test_download_skip_existing(self, downloader, tmp_path):
        ...
        with patch.object(downloader, "validate_audio_file", new_callable=AsyncMock) as mock_val:
            ...
            res = await downloader.download(
                SearchResult("vid", "Title", 100, "url"), TrackMetadata("id2", "Title", "Artist")
            )
    ...
    async def test_transcode_calls_ffmpeg(self, downloader):
        ...
        with patch("asyncio.create_subprocess_exec") as mock_exec:
            ...
            with patch("pathlib.Path.exists", side_effect=[True, True, True]), patch("pathlib.Path.stat") as mock_stat:
