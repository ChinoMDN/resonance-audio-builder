...
class TestAppExpanded:
    ...
    def app(self):
        with patch("resonance_audio_builder.core.builder.Config.load") as mock_load, \
             ...
             patch("resonance_audio_builder.core.builder.ProgressDB") as mock_db_cls:
    ...
    def test_collect_tracks(self, app, tmp_path):
        ...
        csv1.write_text("Track URI,Track Name,Artist Name,Album Name,Disc Number,Track Number,Track Duration (ms),Added By,Added At,ISRC\nspotify:track:123,Song1,Artist1,Album1,1,1,1000,User,Date,ISRC1", encoding="utf-8")
        
        csv2 = tmp_path / "list2.csv"
        csv2.write_text("Track URI,Track Name,Artist Name,Album Name,Disc Number,Track Number,Track Duration (ms),Added By,Added At,ISRC\nspotify:track:456,Song2,Artist2,Album2,1,1,2000,User,Date,ISRC2", encoding="utf-8")
    ...
    def test_start_download_flow(self, mock_async_run, mock_mgr_cls, app, tmp_path):
        ...
        with patch.object(app, "_get_selected_csvs", return_value=[str(csv1)]), \
             patch.object(app, "_select_quality"), \
             patch("resonance_audio_builder.core.builder.console.input") as mock_input:
    ...
    def test_run_watch_mode_cli(self, app):
        with patch.object(sys, "argv", ["script.py", "--watch", "CustomFolder"]), \
