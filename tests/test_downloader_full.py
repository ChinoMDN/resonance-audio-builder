...
class TestDownloaderFull:
    ...
    async def test_perform_transcoding_pipeline(self, downloader):
        ...
        with patch.object(Path, "stat") as mock_stat:
            ...
            success, bytes_n = await downloader._perform_transcoding_pipeline(
                raw, hq, mob, track, True, True
            )
    ...
    async def test_download_via_ytdlp_download_error(self, downloader, tmp_path):
        ...
        with patch("yt_dlp.YoutubeDL") as mock_ydl_cls:
            ...
            with patch("resonance_audio_builder.audio.downloader.AudioDownloader._execute_ydl", side_effect=Exception("Boom")):
