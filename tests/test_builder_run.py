...
class TestAppFull:
    ...
    def test_start_download_flow(self, app, tmp_path):
        ...
        with patch.object(app, '_select_quality'):
            with patch('resonance_audio_builder.core.manager.DownloadManager') as mock_mgr_cls:
                ...
                # builder.py: async def start_download_flow ...
                ...
                # I will assume `start_download_flow` is the public async method and `_start_download` might be internal.
                # Let's stick to calling what's available. 
                # If I look at `test_builder_run.py` original (step 449), it called `start_download_flow`.
                # The user fix says `app._start_download`. I'll use `_start_download` if it exists, otherwise adapt.
                # Safest: Use `start_download_flow` if that's what I saw before, OR try `_start_download`.
                # I'll use the user's provided code but verify method name if it fails. 
    ...
    def test_run_cli_args(self, app):
        ...
        with patch("sys.argv", ["main.py", "--watch", "d:/music"]), \
